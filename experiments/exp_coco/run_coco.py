...
from experiments.exp_coco.results_coco import ResultsCoCo, MethodType
...
def run_coco(reps,
    ...
    for fun_form, fun_str in FUN_FORMS:
        exp_identifier = str(SHIFT_TEST) + '_' + str(CONFOUNDING_TEST)  + "_biv_"+ str(bivariate)+'known_n_z_' + str(known_componets) + '_' + fun_str + '_' + str(D_n)
        write_file = open(f"{path}/out_coco/{exp_identifier}_log.csv", "a+")
        write_final_file = open(f"{path}/out_coco/{exp_identifier}_res.csv", "a+")
            ...
            for xnm in test_cases[pattern]:
                ...
                for rep in range(reps):
                    ...
                    for method in METHODS:
                        ...
                        fci_jci, D, Dobs, seed = run_fci(dag, Dobs, D_n, fun_form, method, FCI_INDEP_TEST, seed) #validates data
...
def run_fci(dag, Dobs, D_n, fun_form, method,
            FCI_INDEP_TEST, seed):
    val_error = False
    try:
