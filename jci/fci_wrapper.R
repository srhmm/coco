...
fci_wrapper <- function(data,systemVars,contextVars,alpha=1e-2,verbose=0,subsamplefrac=0.0,mode='obs',test='gaussCIcontexttest',obsContext=matrix(0,1,length(contextVars)),doPDsep=TRUE,fixedGaps=NULL, fixedEdges=NULL) {
  # data:          Nxp matrix
  ...
  if( mode == 'jci123' || mode == 'jci123r' || mode == 'jci1' || mode == 'jci0' ) {
    ...
    pag<-fcijci(labels=colnames(data[,systemVars]),suffStat,indepTest,alpha=alpha,verbose=verbose,doPdsep=doPDsep,selectionBias=FALSE,fixedGaps=fixedGaps[systemVars,][,systemVars], fixedEdges=fixedEdges[systemVars,][,systemVars])
