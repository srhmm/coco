...
EXPERIMENT = 'dagsearch'

#tag = '_paper'
tag = '_plot'

df = pd.read_csv(f'{RESULTS_DIR}/{EXPERIMENT}{tag}.csv', sep=',', engine='python')
...
df['Recall'] = df['TP'] / (df['TP'] + df['TN'])
df['Precision'] = df['TP'] / (df['TP'] + df['FP'])
#df['F1'] = df['Recall'] * df['Precision'] / (df['Recall'] + df['Precision'])

#normalize SIDs
df['sid'] = df['sid'] / (df['n_variables'] * df['n_variables'])
...
metrics = ['shd', 'sid']
methods = [#'Pooled_PC_KCI', 'MSS_KCI',  r'MC',
           'LINC_rff', 'LINC_gp' ]
