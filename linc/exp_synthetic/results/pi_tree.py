...
from out import Out
from pi_search_clustering import pi_search_clustering
from pi_search_mdl import pi_mdl, pi_normalized_gain, pi_mdl_conf, pi_mdl_best
...
from utils_pi import pi_convertfrom_pair, pi_enum
from vsn import Vsn
...
class PiTree:
    ...
    def eval_edge(self, edge: PiEdge =None,  j = None, i=None)-> (int, int, int, list, list):
        """
        ...
        gain, score_up, mdl_up, pi_up, pa_up = self._edge_gain(i, j)
    ...
    def _edge_gain(self, i, j)-> (int, int, int, list, list):
        """
        ...
        pa_up = [n for n in range(len(self.nodes)) if n in pa_cur or n == i]

        score_up, mdl_up, pi_up = edge_gain_target(self.XYc, covariates=pa_up, target=j, vsn=self.vsn, C_n=self.C_n)

        gain = (score_cur - score_up)
        return gain, score_up, mdl_up, pi_up, pa_up


    def initial_edges(self, q : UPQ, out : Out, subsample_size : int, cmp_score=True, score_cache=None, mdl_cache=None, pi_cache=None)-> (UPQ):
        """
        ...
        if score_cache is not None:
            ...
            self.mdl_cache= mdl_cache
            self.pi_cache= pi_cache
        ...
        for target in self.nodes:
            ...
            if self.score_cache.__contains__(hash_key):
                assert self.mdl_cache.__contains__(hash_key) and self.pi_cache.__contains__(hash_key)
                score_0 = self.score_cache[hash_key]
                mdl_0 = self.mdl_cache[hash_key]
                pi_0 = self.pi_cache[hash_key]
            else:
                ...
                if self.vsn.vario_in_tree_search:
                    ...
                    gpr_0 = PI(Xic0, Yc, np.random.RandomState(1), skip_regression_pairs=not self.vsn.regression_per_pair, rff=self.vsn.rff)
                    gpr_0.cmp_distances()
    ...
    def eval_adj(self, adj):
        ...
        for j in self.nodes:
            ...
            _, mdl_j, _ = self.eval_edge(edge=None, pa=pa, j=j)
            mdl = mdl + mdl_j
        return mdl
...
def initial_edges_target(XYc, target: int,
    ...
    for node_i in covariates:
        ...
        if vsn.vario_in_tree_search:
            ...
            gpr = PI(Xic, Yc, np.random.RandomState(1), skip_regression_pairs=not vsn.regression_per_pair, rff=vsn.rff)
            gpr.cmp_distances()
                ...
                if vsn.clustering:
                    pi, mdl_score, _, _, _ = \
                        pi_search_clustering(gpr.pair_mdl_gains, C_n, gpr,
                                             vsn.regression_per_group, subsample_size)
                else:
...
def edge_gain_target(XYc, covariates, target:int,
    ...
    if vsn.vario_in_tree_search:
        ...
        gpr = PI(Xc, Yc, np.random.RandomState(1), skip_regression_pairs=not vsn.regression_per_pair, rff=rff)
        gpr.cmp_distances()
        if vsn.ilp_in_tree_search:
            if vsn.clustering:
                pi, mdl_score, _, _, _ = \
