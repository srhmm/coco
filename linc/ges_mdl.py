...
import utils_ges_mdl
from score_mdl import MDLScoreFunction
from utils_ges_mdl import score_g_mdl
from vsn import Vsn


def GesMDL(X, #: ndarray,
           vsn : Vsn,
           maxP: Optional[float] = None,
           parameters: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Perform greedy equivalence search (GES) algorithm
    Adapted from the causallearn package, to use the MDL scoring function
    Parameters
    ----------
    X : data set (numpy ndarray), shape (n_samples, n_features). The input data, wshere n_samples is the number of samples and n_features is the number of features.
    ...
    """
    if X.shape[0] > 10:
        warnings.warn("More than 10 contexts")
    if X.shape[1] < X.shape[2]:
        warnings.warn("The number of features is much larger than the sample size!")
    #print("GES on ", X.shape[0], "contexts,", X.shape[2], "variables, ", X.shape[1], "samples")
    ...
    #score_func = MDLPairwiseScoreFunction(Data_c=X,subsample=100)
    score_func = MDLScoreFunction(Data_c=X, vsn=vsn, use_bic=False)
    ...
    score = score_g_mdl(X, G, score_func, parameters)  # initialize the score
                        ...
                        if (S[k] < 2):  # S indicate whether we need to check subset(k)
                            ...
                            if (V1):
                                ...
                                if (V2):
                                    ...
                                    S[np.where(Idx == 1)] = 1
                                    chscore, desc, record_local_score = utils_ges_mdl.insert_changed_score(X, G, i, j, sub[k],
                                                                                                           record_local_score,
                                                                                                           score_func,
                                                                                                           parameters)
            ...
            for j in range(N):
                if ((G.graph[j, i] == Endpoint.TAIL.value and G.graph[i, j] == Endpoint.TAIL.value)
                    ...
                    for k in range(len(sub)):
                        ...
                        if (V):
                            chscore, desc, record_local_score = utils_ges_mdl.delete_changed_score(X, G, i, j, sub[k],
