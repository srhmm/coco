...
class GaussianProcessRFFPi():
    ...
    def fit(self, X_train, y_train):
        ...
        K_dual[np.diag_indices_from(K_dual)] += (1e-1)  # s.t. MDL scores positive and K pos. definite
        mat = np.eye(X_feat.shape[0]) + K_dual * sigma ** -2
        alpha_dual1 = np.linalg.solve(mat, X_feat @ y_train.reshape(-1,1))[:, 0]
        self.alpha_ = alpha_dual1.reshape(-1,1)
        mdl_penalty = alpha_dual1.T @ mat @ alpha_dual1
       ...
       # mdl_penalty= alpha_dual.T @ K_dual @ alpha_dual
        ...
        mean_pred = sigma ** -2 * X_feat.T @ alpha_dual1 #np.linalg.solve(mat, X_feat @ self.y_train_.reshape(-1,1))[:, 0]
